...
export const useQueryProducts = (options = {}) => {
	...
	const query = useQuery({
		...
		retryDelay: import.meta.env.TEST
			? 0
		...
		if (page < query.data?.totalPages) {
			...
			const fetchNextPage = async () => {
				try {
					...
					// Gagal prefetch bukan "data kosong". Lempar agar React Query menandai
					// query error (data tetap undefined) dan halaman tujuan fetch ulang,
					// bukan mengisi cache dengan null yang tampak seperti hasil sukses.
